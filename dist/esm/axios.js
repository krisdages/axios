...
function fetchAdapter(config) {
  return new Promise(function dispatchFetchRequest(resolve, reject) {
    if (noFetch) {
      ...
      if (noFetch) {
        reject(new AxiosError$1("window.fetch not available", "ERR_FETCH_UNAVAILABLE", config, null));
        return;
      }
    }
    ...
    async function handleLoad(response) {
      try {
        // Prepare the response
        const responseHeaders = new AxiosHeaders$2();
        if (response.headers) {
        ...
        if (dataError) {
          reject(new AxiosError$1('Unknown responseType ' + responseType, "ERR_UNKNOWN_RESPONSE_TYPE", config, request, axiosResponse));
        }
    ...
    function handleError(thrown, rawResponse) {
      try {
        if (thrown instanceof DOMException && thrown.name === "AbortError") {
          const err = new CanceledError$1(typeof thrown.reason === "string" ? thrown.reason : null, config, request);
          err.response = rawResponse;
          err.abortError = thrown;
          reject(err);
        }
        else if (isNetworkError(thrown)) {
          const err = new AxiosError$1('Network Error', AxiosError$1.ERR_NETWORK, config, request, rawResponse);
          err.cause = thrown;
          reject(err);
        }
        else {
          reject(AxiosError$1.from(
              thrown,
              thrown.name === "SyntaxError" ? AxiosError$1.ERR_BAD_RESPONSE : null,
              config,
              request,
              rawResponse
          ));
