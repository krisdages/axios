...
forEachAdapter('basicAuth', function () {
    ...
    it('should accept HTTP Basic auth with username/password', function (done) {
      ...
      setTimeout(function () {
        ...
        expect(getRequestHeader(request, 'Authorization')).toEqual('Basic QWxhZGRpbjpvcGVuIHNlc2FtZQ==');
        done();
      }, 100);
    });
    ...
    it('should accept HTTP Basic auth credentials without the password parameter', function (done) {
      ...
      setTimeout(function () {
        const request = jasmine.Ajax.requests.mostRecent();
        expect(getRequestHeader(request, 'Authorization')).toEqual('Basic QWxhZGRpbjo=');
        done();
      }, 100);
    });
    ...
    it('should accept HTTP Basic auth credentials with non-Latin1 characters in password', function (done) {
      ...
      setTimeout(function () {
        ...
        expect(getRequestHeader(request, 'Authorization')).toEqual('Basic QWxhZGRpbjpvcGVuIMOfw6fCo+KYg3Nlc2FtZQ==');
        done();
      }, 100);
    });
