...
it('should not set xsrf header if cookie is null', function (done) {
  ...
  getAjaxRequest().then(function (request) {
    expect(getRequestHeader(request, axios.defaults.xsrfHeaderName)).toEqual(undefined);
    done();
  });
});
...
it('should set xsrf header if cookie is set', function (done) {
  ...
  getAjaxRequest().then(function (request) {
    expect(getRequestHeader(request, axios.defaults.xsrfHeaderName)).toEqual('12345');
    done();
  });
});
