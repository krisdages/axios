forEachAdapter('requests', function () {
beforeEach(function () {
...
function expectReasonRequest(reason) {
  if (axios.defaults.adapter === axios.defaults.allAdapters.fetch) {
    expect(reason.request).toEqual(jasmine.any(Request));
  } else {
...
it('should allow overriding Content-Type header case-insensitive', function (done) {
  ...
  getAjaxRequest().then(function (request) {
    expect(getRequestHeader(request, 'Content-Type')).toEqual(contentType);
    done();
  });
});
...
it('should support URLSearchParams', function (done) {
  ...
  getAjaxRequest().then(function (request) {
    expect(getRequestHeader(request, 'Content-Type')).toBe('application/x-www-form-urlencoded;charset=utf-8');
